...
import { AddUpdateIconModalComponent } from 'src/app/icon/icon/icon-management/add-update-icon-modal/add-update-icon-modal.component';
import { BsModalRef, BsModalService } from 'ngx-bootstrap/modal';
import { ConfirmationModalComponent } from 'src/app/modals/confirmation-modal/confirmation-modal.component';
@Component({
  selector: 'icon-management',
  templateUrl: './icon-management.component.html',
  styleUrls: ['./icon-management.component.css']
})
export class IconManagementComponent implements OnInit {
  ...
  constructor(private IconService: IconService, private commonHelper: CommonHelper,
  ...
  getSlidersWithFilters(event) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.IconService.getAllIcon(params).subscribe((res: any) => {
  ...
  onClickDeleteSlider(slider) {
    ...
    var tempSubObj: Subscription = this.modalService.onHide.subscribe(() => {
      if (this.modalRef.content.decision == "done") {
        this.loading = true;
        this.IconService.deleteIcon(slider._id).subscribe((res: any) => {
          this.loading = false;
          if (res.status == 200) {
            remove(this.table_data, (ub: any) => ub._id == slider._id);
            this._toastMessageService.alert("success", "Icon deleted successfully.");
          }
        }, (error) => {
          this.loading = false;
          this.commonHelper.showError(error);
  ...
  showAddSliderModal() {
    this.modalRef = this.modalService.show(AddUpdateIconModalComponent, { class: 'add-update-slider-modal', backdrop: 'static', keyboard: false });
    this.modalRef.content.decision = "";
