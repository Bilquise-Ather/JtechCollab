...
@Component({
  selector: 'tiles-19',
  templateUrl: './tiles-19-management.component.html',
  styleUrls: ['./tiles-19-management.component.css']
})
export class Tiles19ManagementComponent implements OnInit {
  ...
  getData() {
    return new Promise((resolve, reject) => {
      this.tilesService.getTiles19({}).subscribe(
        (res: any) => {
  ...
  addTiles() {
    ...
    let params = {
      ...
      videolink: this.setting_Obj.videolink,
    }
    data.append('body', JSON.stringify(params));
    this.loading = true;
    this.tilesService.addTiles19(data).subscribe((res: any) => {
      this.loading = false;
      if (res.status == 200 && res.data) {
        this.newImage1Uploaded = false;
