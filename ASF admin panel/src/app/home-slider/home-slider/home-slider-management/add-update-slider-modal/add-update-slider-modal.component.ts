...
@Component({
  selector: 'add-update-slider-modal',
  templateUrl: './add-update-slider-modal.template.html',
  styleUrls: ['./add-update-slider-modal.component.css']
})

export class AddUpdateSliderModalComponent extends BaseModalComponent implements OnInit {
  ...
  addSlider() {
    ...
    this.loading = true;
    this.SliderService.addSlider(data).subscribe((res: any) => {
      this.loading = false;
      if (res.status == 200 && res.data) {
        this._toastMessageService.alert("success", "Slider added successfully.");
  ...
  updateSlider(){
    ...
    data.append('body', JSON.stringify(params));
    this.loading = true;
    this.SliderService.updateSlider(this.slider_obj._id,data).subscribe((res: any) => {
      this.loading = false;
      if (res.status == 200 && res.data) {
        this._toastMessageService.alert("success", "Slider updated successfully.");
        this.dialogResult = res.data;
        this.done();
      }
    }, (error) => {
