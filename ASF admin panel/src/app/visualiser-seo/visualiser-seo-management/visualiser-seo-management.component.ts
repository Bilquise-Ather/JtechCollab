...
@Component({
  selector: 'visualiser-seo-management',
  templateUrl: './visualiser-seo-management.component.html',
  styleUrls: ['./visualiser-seo-management.component.css']
})
export class VisualiserSeoManagementComponent implements OnInit {
  ...
  getData() {
    return new Promise((resolve, reject) => {
      this.faqsService.getVisualiserSeo({}).subscribe(
        (res: any) => {
  ...
  addPrivacyPolicy() {
    ...
    this.loading = true;
    this.faqsService.addVisualiserSeo(params).subscribe((res: any) => {
      this.loading = false;
      if (res.status == 200 && res.data) {
