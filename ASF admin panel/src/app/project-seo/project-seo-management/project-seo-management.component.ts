...
@Component({
  selector: 'project-seo-management',
  templateUrl: './project-seo-management.component.html',
  styleUrls: ['./project-seo-management.component.css']
})
export class ProjectSeoManagementComponent implements OnInit {
  ...
  getData() {
    return new Promise((resolve, reject) => {
      this.faqsService.getProjectSeo({}).subscribe(
        (res: any) => {
  ...
  addPrivacyPolicy() {
    ...
    this.loading = true;
    this.faqsService.addProjectSeo(params).subscribe((res: any) => {
      this.loading = false;
      if (res.status == 200 && res.data) {
