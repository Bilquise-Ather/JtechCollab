...
@Component({
  selector: 'gallery-seo-management',
  templateUrl: './gallery-seo-management.component.html',
  styleUrls: ['./gallery-seo-management.component.css']
})
export class GallerySeoManagementComponent implements OnInit {
  ...
  getData() {
    return new Promise((resolve, reject) => {
      this.faqsService.getGallerySeo({}).subscribe(
        (res: any) => {
  ...
  addPrivacyPolicy() {
    ...
    this.loading = true;
    this.faqsService.addGallerySeo(params).subscribe((res: any) => {
      this.loading = false;
      if (res.status == 200 && res.data) {
